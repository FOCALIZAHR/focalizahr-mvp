...
// ════════════════════════════════════════════════════════════════════════════
// SUCCESSION INTELLIGENCE — DEMO v8 (Real Data)
// /dashboard/succession-demo
// ════════════════════════════════════════════════════════════════════════════
// v8: Conectado a API real /api/executive-hub/succession
//     Ciclo auto-detectado, mapeo VulnerabilityRow → Pos
// ════════════════════════════════════════════════════════════════════════════
...
type View = 'portada' | 'matrix' | 'domino'
...
export default function SuccessionDemoPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // 2. Get active cycle
        const cyclesRes = await fetch('/api/admin/performance-cycles', { headers })
        if (!cyclesRes.ok) throw new Error('Error cargando ciclos')
        const cyclesJson = await cyclesRes.json()
        const cycles = cyclesJson.data || cyclesJson.cycles || []
        if (cycles.length === 0) throw new Error('No hay ciclos de evaluación activos')
        const cycleId = cycles[0].id

        // 3. Fetch succession data
        const res = await fetch(`/api/executive-hub/succession?cycleId=${cycleId}`, { headers })
        if (!res.ok) throw new Error('Error cargando sucesión')
        const json = await res.json()

        if (!json.success || !json.data?.vulnerabilityMap) {
          throw new Error('Sin datos de sucesión')
        }

        console.log('[SuccessionDemo] Raw API response:', JSON.stringify(json.data, null, 2))
        const vulnMap = json.data.vulnerabilityMap as VulnerabilityRow[]
        console.log('[SuccessionDemo] vulnerabilityMap count:', vulnMap.length)
        console.log('[SuccessionDemo] vulnerabilityMap rows:', vulnMap.map(r => ({
          title: r.positionTitle,
          flight: r.flightRisk,
          bench: r.benchStrength,
          candidate: r.bestCandidateName,
          chain: r.chainStatus,
          urgency: r.urgency,
        })))
        const mapped = vulnMap.map(vulnRowToPos)
        console.log('[SuccessionDemo] Mapped positions:', mapped.length)
        setPositions(mapped)
      } catch (err: any) {
        console.error('[SuccessionDemo] Load error:', err)
  ...
  const criticals = useMemo(() => DATA.filter(p => p.urgency === 'CRITICAL'), [DATA])
  const urgents = useMemo(() => DATA.filter(p => p.urgency === 'URGENT'), [DATA])
  const needsAttention = useMemo(() => DATA.filter(p => p.urgency === 'NEEDS_ATTENTION'), [DATA])
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-slate-500 font-light">Cargando inteligencia de sucesión...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center max-w-sm mx-auto p-8">
          ...
          <h2 className="text-lg font-light text-white mb-2">Sin datos de sucesión</h2>
          <p className="text-sm text-slate-500 mb-6">{loadError || 'No hay posiciones críticas configuradas.'}</p>
          <button
            onClick={() => router.push('/dashboard/succession')}
            className="text-cyan-400 hover:text-cyan-300 text-sm flex items-center gap-2 mx-auto transition-colors"
          >
            <ArrowLeft className="w-4 h-4" /> Ir a Gestión de Sucesión
          </button>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white overflow-x-hidden">
      {/* Ambient — warmer, visible */}
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute -top-40 left-1/4 w-[600px] h-[600px] bg-cyan-500/[0.06] rounded-full blur-[120px]" />
        <div className="absolute -bottom-40 right-1/3 w-[500px] h-[500px] bg-purple-500/[0.06] rounded-full blur-[120px]" />
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'matrix' && (
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className="py-8 sm:py-10 space-y-5"
            >
              <div className="flex items-center justify-between">
                <button onClick={() => setView('portada')} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-300 text-xs transition-colors">
          ...
          {view === 'domino' && (
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: -20 }}
              className="py-8 sm:py-10 space-y-8"
            >
              <div className="flex items-center justify-between">
                <button onClick={() => setView('matrix')} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-300 text-xs transition-colors">
...
function NavPill({ active, onChange }: { active: string; onChange: (v: string) => void }) {
  const tabs = [
    { key: 'matrix', icon: Shield, label: 'Mapa' },
    { key: 'domino', icon: Link2, label: 'Dominó' },
  ]
  return (
