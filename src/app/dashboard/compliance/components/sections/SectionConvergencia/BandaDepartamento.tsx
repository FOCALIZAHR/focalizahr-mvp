...
import { classifyForDisplay } from '@/app/dashboard/compliance/lib/format';
...
function BloquesExpandidos({
  ...
  const narrativasCasos: Record<string, string> = {
    ...
    A3: (() => {
      const { display, label } = classifyForDisplay(dims.P2_seguridad);
      const valueStr = display !== null ? `${display} · ${label}` : '—';
      return `El lenguaje que circula en este departamento tiene sesgo. La seguridad psicológica está en ${valueStr}. Cuando el lenguaje normaliza la exclusión, el ambiente que lo permite ya existe.`;
    })(),
    A4: dept.a4Partner
