...
// ════════════════════════════════════════════════════════════════════════════
// TAC HEADER — Zona Cero: Titular Dinamico + Mision + Smart Router Toggle
// src/components/talent-actions/TACHeader.tsx
//
// Principio: El CEO debe ENTENDER en 3s, DECIDIR en 10s, ACTUAR en 1 clic
// buildHeadline → que esta pasando (priorizado por impacto)
// buildMission → que hacer ahora (accion especifica)
// Smart Router → CTA principal + link secundario
// ════════════════════════════════════════════════════════════════════════════
...
interface TACHeaderProps {
  orgMap: OrgMapResult
  activeView: 'gerencias' | 'personas'
  onChangeView: (view: 'gerencias' | 'personas') => void
}

// ════════════════════════════════════════════════════════════════════════════
// HEADLINE + MISSION BUILDERS
// ════════════════════════════════════════════════════════════════════════════
...
function computeAggregates(gerencias: GerenciaMapItem[]) {
  ...
  for (const g of gerencias) {
    ...
    motorTotal += g.riskDistribution.MOTOR_EQUIPO
  ...
  return { plTotal, fugaTotal, potencialPerdido, gerenciasFragiles, motorTotal, gerenciaMasCritica }
}

function buildHeadline(orgMap: OrgMapResult, agg: ReturnType<typeof computeAggregates>): string {
  // PRIORIDAD 1: Dinero en riesgo (>$100M CLP)
...
function buildMission(orgMap: OrgMapResult, agg: ReturnType<typeof computeAggregates>): string {
  // PRIORIDAD 1: Gerencia critica identificada
  ...
  // PRIORIDAD 2: Patron dominante critico
  if (orgMap.orgStats.patronDominante === 'FRAGIL') {
    return 'Contener fuga antes de perder mas talento critico'
  }
  if (orgMap.orgStats.patronDominante === 'QUEMADA') {
    return 'Revisar cargas de trabajo en equipos sobrecargados'
  }
  if (orgMap.orgStats.patronDominante === 'ESTANCADA') {
    return 'Activar desarrollo en equipos con potencial dormido'
  }

  // PRIORIDAD 3: Organizacion saludable
  if (orgMap.orgStats.patronDominante === 'SALUDABLE') {
    return 'Reconocer a los equipos que sostienen la operacion'
  }
...
export default function TACHeader({ orgMap, activeView, onChangeView }: TACHeaderProps) {
  ...
  // Smart Router: determinar vista principal
  ...
  return (
    <div className="text-center space-y-4 mb-6">
      ...
      <div className="flex flex-col items-center gap-2 pt-2">
        {/* CTA Principal — navega a lo que dice el label */}
        <button
          onClick={() => onChangeView(primaryView)}
        ...
        {/* Link Secundario — navega al pilar opuesto */}
        <button
          onClick={() => onChangeView(primaryView === 'gerencias' ? 'personas' : 'gerencias')}
