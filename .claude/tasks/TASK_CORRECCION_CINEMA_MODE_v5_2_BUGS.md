# TASK: CORRECCIÓN CINEMA MODE v5.1

## 📋 METADATA
- **Fecha:** 1 Feb 2025
- **Tipo:** Corrección quirúrgica post-evaluación
- **Archivos principales:** src/components/evaluator/cinema/*
- **Dependencias:** Leer `.claude/docs/focalizahr-ui-design-standards.md`

---

## 🔴 BUGS CRÍTICOS ENCONTRADOS EN PRUEBAS (1 Feb 2025)

### BUG 1: ERROR EN PÁGINA SUMMARY - Campo incorrecto
```
TypeError: Cannot read properties of undefined (reading 'evaluatee')
src\app\dashboard\evaluaciones\[assignmentId]\summary\page.tsx (109:61)

data.assignment.evaluatee.fullName  ← INCORRECTO
```

**El API retorna `summary` NO `assignment`:**
```json
{
  "success": true,
  "summary": {           // ← Es "summary", no "assignment"
    "evaluatee": { "fullName": "...", "position": "..." }
  }
}
```

**FIX:** Cambiar todas las referencias de `data.assignment` a `data.summary`:
```tsx
// ANTES (incorrecto):
const displayName = formatDisplayNameFull(data.assignment.evaluatee.fullName)

// DESPUÉS (correcto):
const displayName = formatDisplayNameFull(data.summary.evaluatee.fullName)
```

### BUG 2: GAUGE PIXELADO
El gauge del Cinema Mode se ve pixelado. 

**NO crear gauge nuevo.** Usar uno de los existentes:
- `ScoreGauge` en `src/app/dashboard/system/page.tsx` (SVG + motion)
- `MiniGauge` en `src/app/dashboard/system/page.tsx` (arco)
- `EvaluatorProgressCard` en `src/components/evaluator/EvaluatorProgressCard.tsx` (Recharts)

### BUG 3: CARD COMPLETADAS SIN NOTA/CATEGORÍA
Las evaluaciones completadas no muestran el score promedio ni categoría.

**Actualmente muestra:**
- ✅ Antigüedad
- ✅ Tipo (MANAGER_TO_EMPLOYEE)
- ✅ Completada (fecha)

**FALTA agregar:**
- ❌ SCORE/NOTA - ej: "4.2/5" o "85/100"
- ❌ CATEGORÍA - ej: badge "Excelente", "Competente", "Necesita Mejora"

**Datos disponibles del API summary:**
```json
{
  "summary": {
    "averageScore": 4.2,        // ← USAR ESTO
    "categorizedResponses": {}
  }
}
```

**Categorías sugeridas (basadas en score 1-5):**
```tsx
const getCategory = (score: number) => {
  if (score >= 4.5) return { label: 'Excepcional', color: 'emerald' }
  if (score >= 4.0) return { label: 'Excelente', color: 'cyan' }
  if (score >= 3.5) return { label: 'Competente', color: 'blue' }
  if (score >= 3.0) return { label: 'En Desarrollo', color: 'amber' }
  return { label: 'Necesita Apoyo', color: 'red' }
}
```

**Agregar en SpotlightCard (grid 2x2 con las otras cards):**
```tsx
{/* NUEVA CARD: Score */}
{employee.status === 'completed' && employee.avgScore && (
  <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
    ...
    <div className="text-2xl font-bold text-cyan-400">
      {employee.avgScore.toFixed(1)}<span className="text-slate-500 text-lg">/5</span>
    </div>
  </div>
)}

{/* NUEVA CARD: Categoría */}
{employee.status === 'completed' && employee.avgScore && (
  <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
    <div className="flex items-center gap-2 text-slate-400 text-xs mb-2">
    ...
    <span className={`px-3 py-1 rounded-full text-sm font-medium bg-${category.color}-500/20 text-${category.color}-400`}>
      {category.label}
    </span>
  </div>
)}
...
**NOTA:** El hook `useEvaluatorCinemaMode` debe fetchear el score para evaluaciones completadas, o el API principal debe incluirlo.

**⚠️ IMPORTANTE - MODIFICAR API:**
El API `/api/evaluator/assignments` actualmente NO retorna `avgScore`. Hay que:

**Opción A (Recomendada):** Modificar el API principal para calcular avgScore en evaluaciones completadas:
```tsx
// En src/app/api/evaluator/assignments/route.ts
// Después de obtener assignments, calcular avgScore para los completados

const mappedAssignments = await Promise.all(assignments.map(async (a) => {
  let avgScore = null;
  
  // Solo calcular para completadas
  if (a.status === 'COMPLETED' && a.participant) {
    ...
    if (scored.length > 0) {
      avgScore = scored.reduce((sum, r) => sum + (r.normalizedScore || 0), 0) / scored.length;
    }
  }
  
  return {
    ...existing_mapping,
    avgScore  // ← AGREGAR ESTE CAMPO
  };
}));
```

**Opción B:** Fetch lazy en el frontend cuando se selecciona una card completada.

---

## 🚫 ARCHIVOS QUE NO DEBES TOCAR (YA FUNCIONAN)

**EVALUACIÓN REAL confirmó que estos YA FUNCIONAN:**
...
**El punto 7 (Página Welcome) está VERIFICADO. NO implementar código del punto 7.**

---

## 🎯 OBJETIVOS (11 puntos - el 7 ya está hecho)

### 🔴 BUGS CRÍTICOS (Hacer PRIMERO)
- **BUG 1:** Fix página Summary - cambiar `data.assignment` → `data.summary`
- **BUG 2:** Gauge pixelado → usar gauge existente del sistema
- **BUG 3:** Mostrar nota/categoría en cards de evaluaciones completadas

### CRÍTICOS (Hacer segundo)
- **Punto 8:** Conectar botón "Ver Resumen" → página Summary
- **Punto 11:** Agregar toast post-envío con sistema FocalizaHR

### VISUALES
- **Punto 1:** Gauge pixelado → usar ScoreGauge existente (RELACIONADO CON BUG 2)
- **Punto 2:** Botón CTA en rail colapsado
- **Punto 3:** Backdrop blur cuando rail expandido
- **Punto 4:** Tabs con colores sólidos premium
- **Punto 5:** Flechas navegación carrusel
- **Punto 6:** Datos completos en SpotlightCard (RELACIONADO CON BUG 3)

### POLISH
- **Punto 9:** VictoryScreen con confetti
- **Punto 10:** Estado vacío
- **Punto 12:** Breadcrumbs (opcional)
...
├── .claude/docs/focalizahr-ui-design-standards.md  ← Estilos UI
├── src/components/ui/toast-system.tsx              ← Sistema toasts (punto 11)
├── src/components/ui/PremiumButton.tsx             ← Botones premium
├── src/app/api/evaluator/assignments/route.ts      ← API evaluator

GAUGES EXISTENTES (NO crear nuevos, usar estos):
├── src/app/dashboard/system/page.tsx               ← ScoreGauge y MiniGauge
├── src/components/evaluator/EvaluatorProgressCard.tsx ← Gauge semicircular Recharts

API SUMMARY (verificar estructura):
├── src/app/api/evaluator/assignments/[id]/summary/route.ts
│   └── Retorna: { success, summary: { evaluatee, averageScore, categorizedResponses } }
```
