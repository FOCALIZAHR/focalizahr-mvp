...
┃  │                                                                    │  ┃
┃  │  💡 LA REVELACIÓN                                                  │  ┃
┃  │     Hipótesis RRHH vs Realidad Encuesta                           │  ┃
┃  │                                                                    │  ┃
┃  │  ┌─────────────────────────┐  vs  ┌─────────────────────────┐     │  ┃
┃  │  │ 📋 HIPÓTESIS RRHH       │      │ 🔍 REALIDAD ENCUESTA    │     │  ┃
┃  │  │    (formulario)         │      │    (confidencial)       │     │  ┃
┃  │  │                         │      │                         │     │  ┃
┃  │  │ 1. Mejor oportunidad 45%│      │ 1. Liderazgo      4.8★  │     │  ┃
┃  │  │ 2. Compensación      30%│      │ 2. Flexibilidad   4.2★  │     │  ┃
┃  │  │ 3. Sin especificar   25%│      │ 3. Autonomía      3.8★  │     │  ┃
┃  │  │                         │      │                         │     │  ┃
┃  │  │ (lo que RRHH registró)  │      │ (lo que revelaron)      │     │  ┃
┃  │  └─────────────────────────┘      └─────────────────────────┘     │  ┃
┃  │                                                                    │  ┃
┃  │  ⚡ INSIGHT CLAVE:                                                 │  ┃
┃  │  "RRHH registró 'Mejor oportunidad' en 45% de casos, pero la      │  ┃
┃  │   encuesta reveló severidad 4.8 en Liderazgo. Se van huyendo      │  ┃
┃  │   del jefe, no buscando nuevas oportunidades."                    │  ┃
┃  │                                                                    │  ┃
┃  └────────────────────────────────────────────────────────────────────┘  ┃
┃                                                                          ┃
...
Muestra el CONTRASTE entre lo que RRHH registró y lo que la encuesta reveló.

### ⚠️ CORRECCIÓN IMPORTANTE

**ANTES (INCORRECTO):**
- "Lo que dicen" = % menciones en encuesta (exitFactors)
- "Lo que duele" = severidad en encuesta (exitFactorsDetail)
- ❌ Ambos vienen de la misma fuente

**AHORA (CORRECTO):**
- "Hipótesis RRHH" = exitReason (formulario de inscripción) 
- "Realidad Encuesta" = exitFactors + severidad (encuesta confidencial)
- ✅ Contrasta DOS fuentes diferentes

### Fuentes de Datos

| Campo DB | Fuente | Momento | Concepto |
|----------|--------|---------|----------|
| `exitReason` | Formulario registro | Al registrar salida | Lo que RRHH cree |
| `exitFactors` | Encuesta P2 | Colaborador responde | Lo que revelan |
| `exitFactorsDetail` | Encuesta P3 | Colaborador responde | Severidad por factor |
...
│  │     (formulario)        │      │     (confidencial)      │         │
│  │                         │      │                         │         │
│  │  Mejor oportunidad 45%  │      │  Liderazgo        4.8★  │         │
│  │  ████████████████░░░    │      │  ████████████████░░     │         │
│  │                         │      │                         │         │
│  │  Compensación      30%  │      │  Flexibilidad     4.2★  │         │
│  │  ████████████░░░░░░░    │      │  ██████████████░░░░     │         │
│  │                         │      │                         │         │
│  │  Sin especificar   25%  │      │  Autonomía        3.8★  │         │
│  │  ██████████░░░░░░░░░    │      │  ████████████░░░░░░     │         │
│  │                         │      │                         │         │
│  │  (lo que RRHH registró) │      │  (lo que revelaron)     │         │
│  │                         │      │                         │         │
...
│  │                                                                   │ │
│  │ "RRHH registró 'Mejor oportunidad' en 45% de casos, pero la     │ │
│  │  encuesta reveló que 'Liderazgo' tiene severidad 4.8 (crítico). │ │
│  │  Se van huyendo del jefe, no buscando nuevas oportunidades."     │ │
│  │                                                                   │ │
│  └──────────────────────────────────────────────────────────────────┘ │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
...
interface RevelationCardProps {
  // Lo que RRHH registró (formulario de inscripción)
  hrHypothesis: {
    reasons: Array<{
      reason: ExitReason;      // 'mejor_oportunidad', 'compensacion', etc.
      label: string;           // 'Mejor oportunidad laboral'
  ...
  // Lo que la encuesta reveló (confidencial)
  surveyReality: {
    factors: Array<{
      factor: string;          // 'Liderazgo de Apoyo'
      avgSeverity: number;     // 1-5 escala
      mentions: number;        // Cantidad que lo mencionaron
    }>;
...
// Calcular distribución de exitReason (Hipótesis RRHH)
function calculateHRHypothesis(exitRecords: ExitRecord[]) {
  const counts = exitRecords.reduce((acc, record) => {
...
function generateInsight(hrHypothesis, surveyReality, onboardingCorrelation) {
  const topHR = hrHypothesis.reasons[0];
  const topSurvey = surveyReality.factors[0];
  
  // Mapear exitReason a factor equivalente para comparar
  const reasonToFactor: Record<string, string> = {
    'problemas_liderazgo': 'Liderazgo de Apoyo',
    'compensacion': 'Compensación y Beneficios',
    'crecimiento_carrera': 'Oportunidades de Crecimiento',
    'balance_vida_trabajo': 'Flexibilidad y Equilibrio',
    // ...
  };
  
  const hrFactorEquivalent = reasonToFactor[topHR.reason];
  
  // Si NO coinciden = REVELACIÓN (el caso más interesante)
  if (hrFactorEquivalent !== topSurvey.factor) {
    let insight = `RRHH registró "${topHR.label}" en ${topHR.percentage}% de casos, `;
    insight += `pero la encuesta reveló que "${topSurvey.factor}" `;
    insight += `tiene severidad ${topSurvey.avgSeverity.toFixed(1)} (`;
    insight += topSurvey.avgSeverity >= 4 ? 'crítico' : 
               topSurvey.avgSeverity >= 3 ? 'alto' : 'moderado';
    insight += '). ';
    
    // Narrativa específica por factor
    if (topSurvey.factor.includes('Liderazgo')) {
      insight += 'Se van huyendo del jefe, no buscando nuevas oportunidades.';
    } else if (topSurvey.factor.includes('Flexibilidad')) {
      insight += 'El desbalance crónico los agota, no es solo dinero.';
    }
    
    if (onboardingCorrelation?.percentage > 0) {
      insight += ` El ${onboardingCorrelation.percentage}% tenía alertas de onboarding ignoradas.`;
    }
  ...
  return `"${topSurvey.factor}" es consistente tanto en el registro de RRHH ` +
         `(${topHR.percentage}%) como en severidad de encuesta ` +
         `(${topSurvey.avgSeverity.toFixed(1)}). El problema está claramente identificado.`;
}
```
...
const hrHypothesis = {
  reasons: [
    { reason: 'mejor_oportunidad', label: 'Mejor oportunidad', count: 9, percentage: 45 },
    { reason: 'compensacion', label: 'Compensación', count: 6, percentage: 30 },
    { reason: 'sin_especificar', label: 'Sin especificar', count: 5, percentage: 25 }
  ],
  totalRecords: 20
};
...
const surveyReality = {
  factors: [
    { factor: 'Liderazgo de Apoyo', avgSeverity: 4.8, mentions: 12 },
    { factor: 'Flexibilidad y Equilibrio', avgSeverity: 4.2, mentions: 10 },
    { factor: 'Autonomía y Confianza', avgSeverity: 3.8, mentions: 8 }
  ],
  totalResponses: 18
};

// Output
const insight = "RRHH registró 'Mejor oportunidad laboral' en 45% de casos, " +
                "pero la encuesta reveló que 'Liderazgo de Apoyo' tiene severidad " +
                "4.8 (crítico). Se van huyendo del jefe, no buscando nuevas oportunidades.";
```
