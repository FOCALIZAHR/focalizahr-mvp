# TASK 03: Crear CinemaSummaryHeader Component

## Objetivo
Crear el componente header para Cinema Summary, reutilizando patrones visuales de SpotlightCard.

## Archivo a Crear
```
src/components/performance/summary/CinemaSummaryHeader.tsx
```

## Contexto
- Debe seguir el MISMO diseño que SpotlightCard existente
- Split 35% identidad / 65% contenido
- Línea Tesla con color dinámico según score
- Avatar con iniciales usando getInitials existente
- Reutilizar PerformanceResultCard existente

## Dependencias Existentes a Usar
```typescript
// YA EXISTEN - solo importar
...
export default memo(function CinemaSummaryHeader({
  ...
  // Score viene en escala 0-100, convertir a 1-5 para clasificación
  const scoreOn5 = score ? score / 20 : null
...
├── PerformanceResultCard.tsx    # YA EXISTE
├── PerformanceScoreCard.tsx     # YA EXISTE
├── PerformanceBadge.tsx         # YA EXISTE
└── summary/                     # CREAR CARPETA
    └── CinemaSummaryHeader.tsx  # ESTE ARCHIVO
```
...
# Verificar imports
# El componente debe importar sin errores desde:
import CinemaSummaryHeader from '@/components/performance/summary/CinemaSummaryHeader'
```

## Criterios de Éxito
- [ ] El componente renderiza sin errores
- [ ] La línea Tesla muestra color dinámico según score
- [ ] El avatar muestra iniciales correctamente
- [ ] El PerformanceResultCard se muestra con el score
- [ ] Los badges de gap analysis se muestran (si hay datos)
- [ ] El botón "Portal" navega a /dashboard/evaluaciones
- [ ] Es responsive (columna en móvil, fila en desktop)

## NO Modificar
- PerformanceResultCard existente
- getInitials existente
