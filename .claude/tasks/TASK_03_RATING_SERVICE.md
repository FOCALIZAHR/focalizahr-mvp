# TASK 03: PERFORMANCE RATING SERVICE

## 🎯 OBJETIVO
Crear el service principal para gestión de ratings de performance.

## 📁 ARCHIVO A CREAR
`src/lib/services/PerformanceRatingService.ts`

## ⚠️ DEPENDENCIAS
- TASK_01 completada (modelos Prisma con campos de ponderación)
- TASK_02 completada (performanceClassification.ts con funciones de ponderación)

## 🎯 ARQUITECTURA DE PONDERACIÓN ENTERPRISE

```
Nivel 0: Default FocalizaHR (código)     → 25/25/25/25
Nivel 1: Config Cliente (cuenta)         → Su política corporativa
Nivel 2: Override Ciclo (opcional)       → Casos especiales

Resolución: cycle.override ?? account.config ?? FOCALIZAHR_DEFAULT
```

Este patrón es estándar Enterprise - Workday, SuccessFactors, Lattice funcionan así.

## 📋 INSTRUCCIONES

### PASO 1: Crear el archivo

Crea `src/lib/services/PerformanceRatingService.ts`:
  ...
  static async generateRating(
    ...
    const rating = await prisma.performanceRating.upsert({
      ...
      create: {
        ...
        cycleId,
        employeeId,
        calculatedScore: weightedScore,  // Score PONDERADO
...
npx tsc src/lib/services/PerformanceRatingService.ts --noEmit
```

- [ ] Archivo creado en `src/lib/services/PerformanceRatingService.ts`
- [ ] Sin errores de TypeScript
- [ ] Import de `prisma` funciona
- [ ] Import de `PerformanceResultsService` funciona
- [ ] Import de `performanceClassification` funciona
- [ ] Todos los métodos exportados
...
// Test getConfig (debería retornar default)
const config = await PerformanceRatingService.getConfig('test-account-id')
console.log(config.scaleType) // 'five_level'
console.log(config.levels.length) // 5
```

## ➡️ SIGUIENTE TAREA
`TASK_04_REFACTOR_EXISTING.md`
