...
| Gate 1+ (transversal) | **`Department.responsableId` — resolver de responsable de departamento** sellado as-built (`f873330`+`9373b5e`, 2026-07-11, sin pushear; doc `ARQUITECTURA_RESPONSABLE_DEPARTAMENTO.md`). Adición TRANSVERSAL a Gate 1, **previa a 5C que la consume**. `Department.responsableId → Employee.id` (SetNull + índice + relaciones nombradas DepartmentEmployees/DepartmentResponsable, `db push` aplicado). Resolver `src/lib/services/DepartmentResponsableService.ts` (`resolveDepartmentResponsable`): evalúa el propio departmentId, sube por `parentId` con guard multi-tenant, tope en primer responsable ACTIVO, fallback `Account.adminEmail` (siempre resuelve); **nunca toca `managerId`** (otro eje). Smoke 5/5 (borrado al sellar), tsc+build limpios. **Backfill `backfill-department-responsable.ts` NULL-only: `--commit` NO corrido** (cobertura inmaterial en fixtures dev; queda para la 1ª nómina real vía EmployeeSyncService). Consumido por 5C (`toEmail` del recordatorio `clima_action_reminder`, resuelto FRESCO). GOTCHA: `ComplianceAnalysisOrchestrator.ts:288` usa "managerId" con semántica parentId (colisión de nombre, no tocar). |
