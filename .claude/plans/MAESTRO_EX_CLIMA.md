...
| v3.20 | **Bloque A — sensibilidad-mean del gauge + narrativa de divergencia fav↔mean** sellado as-built (2026-07-13) — Gate 0: `.claude/tasks/ARQUITECTURA_EI_MEAN_SENSIBILIDAD.md`. Cierra la **ceguera de Glint** en el nivel Outcome/EI del gauge del Lobby: la favorabilidad (%) puede mantenerse sana mientras la **media 1-5 cae** (el grupo insatisfecho se vuelve más crítico). **A-additive puro:** cero cambio a `riskZone`/`calcRiskZone`/`CLIMA_TARGET_FAVORABILITY`; `EngagementGauge.tsx` **INTACTO** (sellado, no reabierto — la caption vive en `ClimaMissionControl`, la frase no cabe en el anillo); sin schema, sin `db push`. **Backend (in-flight sesión previa, en este mismo commit):** `calcOrgMean` (media org ponderada por headcount, escala 1-5, hermana de `calcOrgFavorability` pero **NO deriva zona** — riskZone sigue siendo fav sellado) + `engagementMeanMomentum` enabler propagado a `correlationFlags` + hotspot-mean (`isHotspotMean`=eiMean<p25 de means) + teatro-mean (`theatreDetectedMean` con `THEATRE_ENGAGEMENT_MAX_MEAN=3.0`); **todos los flags nuevos OPCIONALES** (backward-compat con insights viejos, optional chaining en readers). **Transporte + UI:** `orgMean`+`orgMeanMomentum` cableados a `ClimaResultsResponse` + threading espejo de `orgMomentum` (hook→orchestrator→MissionControl). **Narrativa de divergencia (`engagementDivergenceNarrative.ts`, PURA):** 2 frases confirmadas Victor sin variación (CASO A = fav sube/se-mantiene + mean baja; CASO B = fav baja/se-mantiene + mean sube); aparece **SOLO en divergencia** (mismo sentido, o sin campaña anterior → footer cae a la línea fav sellada, **cero regresión**). **Decisiones Victor:** (1) constante NUEVA nombrada `CLIMA_DIVERGENCE_MEAN_MIN_DELTA=0.2` — **NO** reusar `REACTIVE_MOMENTUM_MIN_DELTA` (miden cosas distintas aunque hoy compartan valor; desacople evita arrastre en ajustes futuros). (2) Gate favorabilidad = `Math.round(favMomentum)` — **el mismo redondeo de la línea 1 del gauge** (`getFooterText`) → la dirección narrada nunca contradice el entero que el CEO lee arriba, **sin introducir umbral nuevo de favorabilidad**; el rounded-0 ("se mantiene") con mean↓ dispara CASO A (es el núcleo del deterioro oculto, no un edge a suprimir). (3) escala confirmada por código antes de fijar umbrales: `engagementMean`/`orgMean` son **1-5 crudo** (`FavorabilityCalculator:34/81`, comparado directo vs `THEATRE_ENGAGEMENT_MAX_MEAN`/`EI_RISK_MEAN`), por eso el piso mean es **0.2**, no ±1.0 (inalcanzable en 1-5). **Colateral:** el campo requerido `engagementMeanMomentum` agregado a `PulseDeptInput` había dejado sin compilar a `smoke-gate3-alg5-costeo.ts` (único error de `tsc`, NO del trabajo de transporte/UI); ese smoke se retira en este commit (gate ya sellado). **Evidencia:** smoke PURO **29/29** (sin DB → no toca prod), `tsc`+`build` limpios, smoke `smoke-clima-ei-mean.ts` retirado al sellar. **Diferido/fuera de scope:** `riskZone` fav sigue sellado (migrar reabre sello); media absoluta en UI (hoy solo el delta narra); recalibrar 0.2 con datos reales. **Pendiente 5D.** |
| v3.19 | **Severidad/Trigger reactivo + mean score** sellado as-built (2026-07-12) — Gate 0: `ARQUITECTURA_SEVERIDAD_REACTIVO_MEAN.md` (Sección B/Cluster A ya no diferidos), as-built técnico `AS_BUILT_SEVERIDAD_REACTIVO_MEAN.md`. **Baja la CAPA DE ACCIÓN del plan de dimensión+fav a reactivo+mean** (el % favorable es ciego al deterioro dentro de las cajas bajas — Glint/Culture Amp). **A-additive puro:** cero cambio a riskZone/CLIMA_TARGET_FAVORABILITY/selectedReactive; todo Json + constantes NOMBRADAS PROVISIONAL. **4 piezas:** (1) **momentum reactivo** — `prevReactiveScores` (reactiveScores al select de prevInsights + a pulseInputByDept, sin re-query) + `ReactiveImpact.reactiveMomentumDelta/State` (Δmean raw, umbral 0.2 Gallup, sin carry-forward). (2) **Cluster A** (`ClimaActionPlanBuilder` reescrito): dispara si ≥1 reactivo no-circular bajo su tier de mean (gapMean<0); palanca = mayor `priorityMean=|impact|×|gapMean|`; severidad = `reactiveSeverityZone(gapMean)` (mapeo interno −0.3/−0.7, NO calcRiskZone → la señal que dispara narra); `getIntervention` gana `leverOverride` (la variante habla del mismo reactivo; D3 intacto). (3) **fix 5C** (`ActionEffectivenessService`): `DriverImpact.meanMomentumDelta = (current.mean−prev.mean)×25` (reutiliza ±5 sellados), leído en vez de `momentumDelta`-fav; `momentumDelta`-fav intacto para riskZone/Cascada/aggregate. (4) **escalamiento sistémico ≥50%** (`getSystemicIntervention`): si ≥`REACTIVE_SYSTEMIC_RATIO` de reactivos medidos bajo tier → narrativa de dimensión PROVISIONAL ({n}/{total}/{categoría}); detección INDEPENDIENTE de N≥25. **Decisiones Victor:** tier-dict `compensaciones`→`beneficios` (era bug: no es subcategory); circularidad EXCLUYE retencion/recomendacion/orgullo/experiencia_general (mismo constructo que EI, impacto inflado); benchmark FIJO no-promedio-interno (misma razón Gate 3 v3.7, documentado en la constante). **Evidencia:** smoke PURO 20/20 (sin DB → no toca prod), tsc+build limpios, smoke retirado al sellar. **Diferido/fuera de scope:** riskZone fav (migrar reabre sello, vía = señal mean aditiva), Cascada/UI/PDI/suggestDriverFocus (Sección C); recalibrar cortes −0.3/−0.7 con datos reales; orden UI de ítems sistémicos (5D). **ADDENDUM (2026-07-13, auditoría Peakon/Culture Amp/Glint):** **Gap 1 SELLADO** — piso `REACTIVE_MIN_IMPACT=0.20` (Peakon: reactivos < 0.20 = ruido, se retiran de la prioridad sin importar el mean); guard en `priorityMean` (abajo del piso = igual que `impact===null`) + `narrativeLever` condicionado (si ningún reactivo supera el piso → ítem dispara igual pero `selectedReactive=null` → celda default; cierra de paso el hueco de que un `priorityMean`-null igual se nombraba palanca por desempate). Solo `ClimaActionPlanBuilder`+`climaThresholds`; `reactiveAnalysis` intacto. Smoke 25/25. **Gap 2 CONSTRUIDO (2026-07-13)** — Kendall's Tau-c para el impacto REACTIVO (Likert ordinal). Análisis empírico: Tau-c REAL ≈ Pearson (ratio 1.01 sobre 36 insights) → piso 0.20 NO cambia; 6/47 flips de palanca (2 ranking + 4 el mismo `colaboracion` borderline). Nueva `reactiveImpactCorrelation` (`τ_c=2m(P−Q)/n²(m−1)`, ordinaliza a 1-5) reemplaza solo el call de `reactiveImpactsForRows` (`:483`); `calculatePearsonR` compartida INTACTA (`:410` ALG1, `:859` ALG5, `GoalsDiagnosticService:896` Metas×Performance — ahí son promedios, Pearson correcto). Umbral de nulidad reactivo 5→25 (`REACTIVE_LOCAL_MIN_N`, aceptado, revisar con deptos chicos). `REACTIVE_STRENGTH_BANDS` PROVISIONAL (solo referencia). Smoke 37/37. Ver `AS_BUILT_SEVERIDAD_REACTIVO_MEAN.md`. **Pendiente 5D.** |
...
| Gate | Nombre | Estado |
|------|--------|--------|
| 1 | Foundation (Taxonomía + Persistencia + Seguimiento Focalizado) | ✅ SELLADO `28c9369`+`ec2694e`+`7cc04e3` (2026-07-06, smoke 40/40 + E2E filtro vivo) · **+ `Department.responsableId` (transversal) ✅ SELLADO** `f873330`+`9373b5e` (2026-07-11, sin pushear; doc de origen `ARQUITECTURA_RESPONSABLE_DEPARTAMENTO.md`): schema `Department.responsableId → Employee.id` (SetNull + índice + relaciones nombradas DepartmentEmployees/DepartmentResponsable, `db push` aplicado) + resolver `src/lib/services/DepartmentResponsableService.ts` (`resolveDepartmentResponsable`: evalúa el propio departmentId → sube por `parentId` con guard multi-tenant → tope en 1er responsable ACTIVO → fallback `Account.adminEmail`; **nunca toca `managerId`**). Smoke 5/5 (borrado al sellar), tsc+build limpios. **Backfill `backfill-department-responsable.ts` NULL-only: `--commit` NO corrido** (queda para la 1ª nómina real vía EmployeeSyncService). **Previo a 5C, que lo consume** (`toEmail` del recordatorio `clima_action_reminder`, resuelto FRESCO). GOTCHA: `ComplianceAnalysisOrchestrator.ts:288` usa "managerId" con semántica parentId (colisión de nombre, no tocar). |
...
| 5 | Planes de Acción (doble CTA + validación impacto) | 🟡 **5A ✅** `ffce15f` (capa de datos + diccionario 8×4 PROVISIONAL + `ClimaActionPlanBuilder` + fix RBAC clima; smoke 24/24) · **5B-i ✅** `49ba0be` (PDIEngine extensión aditiva: `climaContext?`/`climaEvidence?` + mapeo PROVISIONAL + `buildClimaGapInput`; snapshot idéntico `cf5f860b` + clima 10/10) · **5B-ii ✅ SELLADO** `811c7dd` (2026-07-10, CTA1 endpoint `/api/clima/pdi-suggestion` + `DevelopmentGoal.climaEvidence` schema; CTA2 reusa POST /goals; E2E 12/12 crea PDI+meta reales en BD; Performance PDI cero diff; tsc+build limpios) · **5C ✅ SELLADO** `fa1e014`+`e2a86e5` (2026-07-11, efectividad: `ClimaActionLog` + `ActionEffectivenessService`, smoke 15/15 — changelog v3.17) · **Dynamic Impact Drivers ✅ SELLADO** `8a9db97`+`fae43d8` (2026-07-12, impacto por reactivo, smoke 26/26 — v3.18) · **Severidad/Trigger reactivo+mean ✅ SELLADO** `771e4ae`+`340145f` (2026-07-12, Cluster A + momentum reactivo + fix 5C + sistémico ≥50%, smoke 20/20 — v3.19) · **Capa 2 variantes (93 celdas) ✅ INTEGRADA** (2026-07-17, `ClimaInterventionVariantCell` + 93 celdas transpuestas + muestra migrada + `climaProductDispatcher.ts` mapa declarativo, smoke 35/35 — v3.21; META wiring pend. 5D Tab 2) · **Narrativas sistémicas (8 + fallback) ✅ SELLADO** `7e96821` (2026-07-20, tercer shape `ClimaSystemicCell` + `SYSTEMIC_INTERVENTIONS` 8 + `getSystemicFallback`, `SIN_CTA` en las 9, `reconocimiento`/`compensaciones` inertes por 1 reactivo, smoke 36/36 — v3.22). **Solo 5D pendiente** (Cinema Mode planes; el reactivo+mean se validará contra datos reales ahí o en un Seguimiento Focalizado). **Toda la cadena de clima está en `origin/main`** (verificado 2026-07-17: Dynamic Impact `8a9db97`+`fae43d8`, Severidad reactivo+mean `771e4ae`+`340145f`, Bloque A `32de7ff`+`0d1b5c2`, Capa 2 `9b48cd4`+`7d87cf5` son ancestros de `origin/main`) |
...
> ⚠️ **AS-BUILT — refinamientos post-5A (leer junto a esta spec de diseño):** la capa de
> acción evolucionó de **dimensión+fav** a **reactivo+mean** en 2 gates aditivos posteriores
> (ver changelog): **Dynamic Impact Drivers (v3.18)** — impacto por reactivo (`reactiveScores`/
> `reactiveAnalysis`, rompe el techo de 32 recetas) — y **Severidad/Trigger reactivo+mean (v3.19)** —
> el disparo/severidad/veredicto 5C corren sobre reactivo+mean (gapMean vs tiers, `meanMomentumDelta`),
> NO sobre la favorabilidad de dimensión (ciega al deterioro en cajas bajas). El shape §5A y el
> "diccionario × 4 niveles" de abajo siguen vigentes como BASE; la selección del reactivo-palanca,
> la zona por gapMean y el escalamiento sistémico se agregaron encima. `riskZone` fav intacto (fuera
...
> **Precondiciones duras de 5D (deuda BLOQUEANTE heredada de Capa 2 v3.21):**
>
> - **META_AREA / META_DURA sin runtime (Tab 2).** Los `target` de las variantes Capa 2
>   son **inertes** hasta que `activateProduct(target, ctx)` los resuelva contra
>   `POST /api/goals` en Tab 2 (POR PERSONA). Hoy solo existe el mapa declarativo
>   `CLIMA_PRODUCT_DISPATCH` (`climaProductDispatcher.ts`), que marca ambos targets
>   `pending`. **Si 5D no construye ese handler, los CTA de Meta nacen inertes** — no es
>   opcional, es precondición del Tab 2.
> - **`energia` — decisión de exclusión circular abierta.** Hoy está **VIVO**: NO figura
>   en `REACTIVE_CIRCULARITY_EXCLUDE` (sellado en v3.19 con 4 reactivos —
