...
- **DEUDA COSMÉTICA (menor, ~50 líneas, reaplicable a mano) — `buildLineaTokens` sin tooltip-word:** en `CoverageNarrativeDictionary.ts`, la función interna `buildLineaTokens` (HEAD **línea 242**) compone las líneas por-depto del sub-hallazgo "El silencio que ya habla" (banda viva: `buildSilencioYaHabla` → `BandaSilencioExterna.tsx` → `SectionConvergencia`). Quedó en el patrón viejo de frase plana, mientras el RESTO del mismo archivo ya migró a tooltip-words (`buildNarrativaPrincipal` HEAD **322-324**, `buildCoachingTip` HEAD **411-413** ya usan `{ type: 'tooltip', value: 'salida'/'onboarding', tooltipKey }`). Mejora = alinear `buildLineaTokens` al mismo patrón: (a) el bloque exit-sin-factor (HEAD **256-280**, hoy compone vía `EXIT_PHRASE_BY_ALERTTYPE` decl. **línea 91** + `ALERT_TYPE_LEGAL` **línea 103**) → reescribir como `switch (item.exitAlertType)` con la palabra "salida" como token tooltipable (`tooltipKey: 'exit'`) y `{ type: 'legal' }` inline en el caso `ley_karin`; (b) el caso `onboarding` (HEAD **282**) → "onboarding" como tooltip-word. NO toca la deuda de participación (arriba) — `buildLineaTokens` solo maneja `tipoSenal: 'exit' | 'onboarding' | 'otra'` (voz externa), no participación. Pura consistencia interna; si nunca se hace, no rompe nada. (Origen: stash WIP descartado 2026-06-16 — confirmado sin más valor único que esto.)
