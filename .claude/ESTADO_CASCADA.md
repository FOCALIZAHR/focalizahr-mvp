...
- **Gate convergencia "por hecho" — DECIDIDO, pendiente de cableado (sesión nueva con handoff):** la convergencia (`loadDepartmentExternalAlerts` Fase 2, `ConvergenciaEngine.ts:818-819`) filtra por ESTADO (pending/acknowledged). 3 decisiones SELLADAS (Victor): **(a) estado FUERA** — status-agnóstico; **(b) ventana 12m** unificada (alinea con `ALERTAS_WINDOW_MONTHS`/`DENUNCIA_WINDOW_MONTHS`, deja el 6m `HISTORICAL_LOOKBACK_MONTHS` solo para senal_ignorada legacy); **(c) PESO COMPLETO, sin decay** (`getHistoricalWeight` no se aplica). Medido: cmob0e56 NO se mueve (las 5 ExitAlert cerradas en 12m están en Operaciones/Personas/Compensaciones, fuera de los deptos analizados TI/Equipos Médicos; `scripts/measure-cmob-closed-alerts.ts`). ISA inmune (usa `riskSignalsCount` score-based, no `pesoEfectivo` — `Orchestrator:302`). Severidad ×1.3 (`nivelFinal`) SÍ puede moverse al incluir cerradas (no en cmob0e56). Oráculos de los 8 mundos intactos (fixtures). Cableado mínimo: scopear los 2 callers `ConvergenciaEngine:1114` + `Orchestrator:274` a fecha-12m status-agnóstico.
